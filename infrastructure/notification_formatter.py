...
def get_execution_time(start_time, end_time):
    ...
    :param start_time: start time of the ruleset
    :param end_time: end time of the ruleset
    :return: execution runtime of the ruleset
    """
    from datetime import timedelta
    start_time = start_time - timedelta(days=55, hours=25, minutes=33, seconds=41)
...
def lambda_handler(event, context):
    ...
    try:
        if event['detail']['context']['contextType'] == 'GLUE_DATA_CATALOG':
            ...
            subject_text = f"Glue DQ Ruleset - {str(event['detail']['rulesetNames'][0])} run details"
