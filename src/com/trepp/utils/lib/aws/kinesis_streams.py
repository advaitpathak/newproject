...
class KinesisStream:
    ...
    def create(self, name, wait_until_exists=True):
        ...
        """
        try:
            stream_created = self.kinesis_client.create_stream(StreamName=name, ShardCount=1)
            self.name = name
    ...
    def put_record_json(self, data, partition_key):
        """
    ...
    def put_record_avro(self, data, partition_key):
        """
        Puts data into the stream. The data is formatted as JSON before it is passed
        to the stream.
    ...
    def get_records(self, max_records, shard_number=0):
        """
        ...
        """
        try:
            shard_id = self.details['Shards'][shard_number]['ShardId']
            print('reading from shard {}', shard_id)
            response = self.kinesis_client.get_shard_iterator(
                StreamName=self.name, ShardId=shard_id,
