...
def read_athena_query(landing_db, curation_db, presentation_db):
    logging.info("Reading Athena query from athena_query.txt")
    with open('athena_query.txt', 'r') as f:
        athena_query = f.read()
...
def run_athena_query(query, bucket, prefix):
    athena_output_location = f"s3://{bucket}/{prefix}"
...
def process_tables(tables, table_map):
    ...
    for table in tables:
        ...
        mapping = table_map.get(table_name, {})
        ...
        dataset_stats[dataset]["tables"] += 1
        dataset_stats[dataset]["total_columns"] += t_total_cols
        dataset_stats[dataset]["described_columns"] += t_described_cols
        dataset_stats[dataset]["lob"] = lob

    return database_total_tables, database_described_tables, total_columns, described_columns, lob_stats, dataset_stats, table_metrics, missing_tables


def aggregate_metrics_generate_report(database_total_tables, database_described_tables, total_columns,
                                      described_columns, lob_stats, dataset_stats, database_name, table_metrics,
                                      missing_tables):
...
def lambda_handler(event, context):
    try:
        ...
        prefix = f"column-description-coverage/{database_name}/"

        # 1. GET DATASET + LOB MAPPING FROM ATHENA
        athena_rows = run_athena_query(athena_query, bucket, prefix)

        table_map = {}

        for row in athena_rows:
            dataset = row["prefix"]  # archivedb
            lob = row["lob"]

            table_name = row["dataset"].split("/")[-1].lower()
        ...
        # 3. PROCESS TABLES
        database_total_tables, database_described_tables, total_columns, described_columns, lob_stats, dataset_stats, table_metrics, missing_tables = process_tables(
            tables, table_map)
        ...
        report = aggregate_metrics_generate_report(database_total_tables, database_described_tables, total_columns,
                                                   described_columns, lob_stats, dataset_stats, database_name,
                                                   table_metrics, missing_tables)

        # 5. WRITE REPORT TO S3
        timestamp = datetime.now(UTC).strftime("%Y%m%d_%H%M%S")
        key = f"{prefix}coverage_{timestamp}.json"
